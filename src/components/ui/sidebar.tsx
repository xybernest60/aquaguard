...
>(
  (
    ...
    const TriggerIcon = side === 'left' ? ChevronsLeft : ChevronsRight;
    ...
    return (
      <div
        ref={ref}
        className={cn("group hidden md:flex flex-col h-svh transition-all duration-300 ease-in-out",
...
>(({ className, ...props }, ref) => {
  ...
  const TriggerIcon = state === 'expanded' ? ChevronsLeft : ChevronsRight;
  ...
  return (
    <Button
      ...
      {...props}
    >
       {isMobile ? <PanelLeft /> : <TriggerIcon />}
  ...
  (
    ...
    if (state === 'expanded' && !isMobile) {
      ...
      return <Comp 
        ...
        >
          {icon}
          <span className="flex-1 truncate">{rest}</span>
      </Comp>
    }
